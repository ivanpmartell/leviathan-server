...
namespace PTech
{
	internal class Game
	{
		...
		private void AddCampaignContent(GamePlayer p)
		{
			...
			if (pack == null)
			{
				Console.WriteLine("Failed to find campaign content pack " + m_mapInfo.m_contentPack);
			}
			else
			{
		...
		private void CreateNewCampaignFleet(GamePlayer player)
		{
			...
			if (xmlDocument == null)
			{
				Console.WriteLine("Failed to load shipfleet file " + m_mapInfo.m_defaults);
				return;
			}
			...
			if (fleets.Count > 0)
			{
				...
				{
					Console.WriteLine("Missing fleet for " + m_nrOfPlayers + " players in map " + m_levelName + " using fleet 0");
					fleetDef = fleets[0];
				}
				else
				{
		...
		public void SetPlayerFleet(string player, string fleetName)
		{
			...
			if (gamePlayerByName != null)
			{
				...
				if (fleetDef == null)
				{
					Console.WriteLine("failed to find player " + gamePlayerByName.GetUser().m_name + " fleet " + fleetName);
					return;
				}
		...
		public void OnUserDisconnected(User user)
		{
			...
			if (gamePlayerByUser.m_inGame)
			{
				...
				{
					Console.WriteLine("Player " + gamePlayerByUser.m_userName + " disconnected, reseting init request from that player");
		...
		private bool Commit(GamePlayer gplayer, int turn, bool surrender, double planningTime, byte[] orders)
		{
			...
			{
				Console.WriteLine("ERROR: player commited turn " + turn + " but current turn is " + m_currentTurn);
				return false;
			}
			TurnData currentTurnData = GetCurrentTurnData();
			...
			{
				Console.WriteLine("ERROR: player " + gplayer.GetUser().m_name + " already commited turn " + turn);
				return false;
			}
		...
		private void RPC_SimulationResults(RPC rpc, List<object> args)
		{
			...
			if (num2 == m_currentTurn)
			{
				...
				for (int i = 0; i < num3; i++)
				{
					...
					{
						Console.WriteLine("got invalid rank player");
						return;
					}
				...
				{
					Console.WriteLine("Got null data from client, BAD CLIENT ");
					return;
				}
