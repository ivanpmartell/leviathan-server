...
namespace PTech
{
	public class ShipDef
	{
		...
		public bool IsValid(User user, ComponentDB cdb)
		{
			...
			{
				Console.WriteLine("Player " + user.m_name + " uploaded ship " + m_name + " with value missmatch , user value " + m_value + "  server value " + shipValue);
				return false;
			}
			return true;
		}
		...
		private bool IsSectionValid(SectionDef section, List<string> sections, List<string> modules)
		{
			...
			{
				Console.WriteLine(" missing section " + section.m_prefab);
				return false;
			}
			foreach (ModuleDef module in section.m_modules)
			{
				...
				{
					Console.WriteLine(" missing module " + module.m_prefab);
					return false;
				}
			}
			return true;
